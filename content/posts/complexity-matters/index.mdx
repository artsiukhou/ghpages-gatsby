...
software developer just needs to know (and care about) things like `hash_map has constant (in average) complexity on its operations` or `for-loop is linear`, right?
...
def fetch_posts(uuids):
    query = """
    SELECT up.uuid, p.post_data FROM posts p
    JOIN user_post up ON p.post_id=up.post_id
